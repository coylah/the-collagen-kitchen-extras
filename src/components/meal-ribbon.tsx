...
import { Salad, Milk, Soup, Search } from "lucide-react";
...
export const RIBBON_LINKS: { key: string; label: string; icon: typeof Salad; to: string }[] = [
  ...
  { key: "search", label: "Search", icon: Search, to: "/" },
];
 ...
 * rather than each page building its own copy. The Search tile links home
 * with a #search hash; the home page watches for that hash on mount and
 * opens its search box automatically.
 */
      ...
      <div className="mx-auto max-w-6xl grid grid-cols-4">
        {RIBBON_LINKS.map(({ key, label, icon: RibbonIcon, to }) => (
          <Link
            key={key}
            to={to}
            hash={key === "search" ? "search" : undefined}
